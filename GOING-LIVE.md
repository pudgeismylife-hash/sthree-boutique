...
Live is not the same as findable. Google does not discover a new domain on its
own in any useful timeframe: it finds sites through links from pages it already
knows, or through a sitemap somebody submits. A two-day-old domain with no
inbound links has neither, and `site:sthreeboutique.com` returns nothing at all
-- not a poor ranking, no pages whatsoever.

The site's half is done and is generated by the build, so it stays in step with
the address:

  https://sthreeboutique.com/robots.txt
  https://sthreeboutique.com/sitemap.xml

The rest is account work that no build can do.

**Search Console** (search.google.com/search-console) is what starts the clock.
Verification was automatic here -- GoDaddy is a Google domain partner, so
choosing "Domain name provider" verified without a TXT record being typed by
hand. Then two things that are easy to stop short of:

1. **Sitemaps** -> submit `https://sthreeboutique.com/sitemap.xml`
2. **URL Inspection** -> paste the address -> **Request indexing**

The second is what actually puts the page in the queue. Submitting the sitemap
alone is an invitation, not a request.

Expect **days to two or three weeks** for a new domain. Nobody can shorten it,
and anyone selling a service that claims to is selling nothing.

**Do not delete unexplained TXT records at GoDaddy.** Verification lives in one,
and removing it silently un-verifies the property along with everything built on
top of it.

**Google Business Profile** (business.google.com) matters more than the website
for a shop with a street address. It is approved in days rather than weeks, and
it is what puts the boutique on Maps and in the local panel with photographs,
hours and a call button. For "boutique near me" it does more than any amount of
