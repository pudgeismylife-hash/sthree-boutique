...
/* ── the collection page, and a page per category ───────────────── */
/* The front page shows a short selection and a door; everything lives here.
   Built from the same source file with one constant changed, so there is no
...
function collectionPage(html, base) {
  const out = html
    ...
    .replace(/<meta name="description" content="[^"]*">/,
      '<meta name="description" content="Every piece at Sthree Boutique — western and ethnic wear, ' +
      'co-ord sets, anti-tarnish jewellery, press-on nails and bags. Imported and Indian, ' +
      'Bikarnakatte, Mangalore. Order on WhatsApp.">');
  if (out.indexOf('const PAGE = "all";') === -1)
    throw new Error("collection.html: the PAGE constant was not found in the source");
  fs.writeFileSync(DIR + "collection.html", out, "utf8");
  return out.length;
}

/* One page per category: a real address to put in an Instagram bio or send on
   WhatsApp, and something a search engine can index for "anti-tarnish jewellery
   Mangalore". Same shape as the product pages -- correct preview tags, then
   straight through to the collection filtered to that category. */
function categoryPages(items, base) {
  const esc = s => String(s).replace(/[&<>"]/g, c => ({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;"}[c]));
  const dir = DIR + "c/";
  fs.mkdirSync(dir, { recursive: true });
  ...
  if (block) {
    const re = /\{\s*key:"([^"]+)",\s*name:"([^"]+)",\s*note:"([^"]+)"/g;
    let m; while ((m = re.exec(block[1]))) cats.push({ key: m[1], name: m[2], note: m[3] });
  }
  if (!cats.length) throw new Error("c/*.html: could not read the category list");

  let written = 0;
  for (const c of cats) {
    const n = items.filter(p => p.cat === c.key).length;
    const card = "assets/products/cat-" + c.key + ".jpg";
    const hasCard = fs.existsSync(DIR + card);
    const title = c.name + " — Sthree Boutique, Mangalore";
    const desc = (n ? n + (n === 1 ? " piece" : " pieces") : "Nothing in stock just now") +
      " · " + c.note + " · Sthree Boutique, Bikarnakatte, Mangalore. " +
      "Imported and Indian, delivered all over India and worldwide. Order on WhatsApp.";
    const here = base + "/c/" + c.key + ".html";
    const real = base + "/collection.html?cat=" + encodeURIComponent(c.key);
    const rel  = "../collection.html?cat=" + encodeURIComponent(c.key);

    fs.writeFileSync(dir + c.key + ".html",
`<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
...
<meta property="og:description" content="${esc(desc)}">
<meta property="og:url" content="${esc(here)}">
${hasCard ? `<meta property="og:image" content="${esc(base + "/" + card)}">
<meta property="og:image:secure_url" content="${esc(base + "/" + card)}">
<meta property="og:image:type" content="image/jpeg">
<meta property="og:image:alt" content="${esc(c.name)} at Sthree Boutique">` : ""}
<meta name="twitter:card" content="summary_large_image">
<meta name="twitter:title" content="${esc(title)}">
<meta name="twitter:description" content="${esc(desc)}">
${hasCard ? `<meta name="twitter:image" content="${esc(base + "/" + card)}">` : ""}
<style>
body{margin:0;background:#FAF6F0;color:#14110F;font-family:system-ui,"Segoe UI",sans-serif;
  display:grid;place-items:center;min-height:100vh;padding:24px;line-height:1.6}
.c{max-width:420px;text-align:center}
...
<body>
<div class="c">
${hasCard ? `<img src="../${esc(card)}" alt="${esc(c.name)}" width="360" height="480">` : ""}
<h1>${esc(c.name)}</h1>
<p>${esc(c.note)} · ${n ? esc(String(n)) + (n === 1 ? " piece" : " pieces") : "nothing in stock just now"}</p>
<p><a class="b" href="${esc(rel)}">See ${esc(c.name.toLowerCase())}</a></p>
</div>
<script>
location.replace(${JSON.stringify(rel)});
<\/script>
...
`, "utf8");
  ...
  /* A category taken out of the site should not keep an address that a search
     engine still offers and WhatsApp still previews. */
  const keep = new Set(cats.map(c => c.key));
  let swept = 0;
  for (const f of fs.readdirSync(dir)) {
    if (!f.endsWith(".html")) continue;
    if (keep.has(f.slice(0, -5))) continue;
    fs.unlinkSync(dir + f);
    swept++;
  }
  return { written, swept, cats: cats.length };
}
...
console.log("cats   c/*.html (" + cstubs.written + " category pages)" +
