...
def make_thumb(path, transparent=False):
    ...
    A figure much taller than the frame is scaled to the height for the same
    reason. The card is 4:5 and the newer cut-outs are 2:3, so filling one costs
    a sixth of its height off the top and bottom -- which on a full-length model
    is her head and the hem of the piece. A little cream down either side is the
    cheaper loss."""
    if transparent:
        # Cut from the RGBA original so the thumbnail keeps its alpha. Trimming
        # is by the alpha channel here -- trim_box reads a flat border colour and
        # there isn't one. The card and the gallery strip sit on cream-alt, and a
        # thumbnail flattened onto cream shows a visible seam against it.
        im = Image.open(path)
        im.load()
        im = im.convert("RGBA")
        im = im.crop(im.getchannel("A").getbbox() or (0, 0, im.width, im.height))
        mode, ground = "RGBA", (0, 0, 0, 0)
    else:
        im = Image.open(path).convert("RGB")
        im = im.crop(trim_box(im))
        mode, ground = "RGB", CREAM
    tw, th = THUMB_SIZE
...
def rebuild_thumbs():
    ...
    for f in sorted(os.listdir(FULL)):
        ...
        if os.path.exists(alpha):
            if APPLY:
                make_thumb(alpha, transparent=True).save(
                    os.path.join(THUMB, f[:-4] + ".webp"), "WEBP", quality=86, method=6)
            m += 1
    return n, m
